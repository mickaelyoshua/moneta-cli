...
use moneta_cli::models::types::{NonEmptyString, PositiveAmount, TransactionSource, TransactionType};
...
async fn test_transaction_insert_and_find_all(pool: PgPool) {
    ...
    let cat_id: i32 = sqlx::query("INSERT INTO categories (name, category_type) VALUES ('Food', 'expense') RETURNING id")
        .fetch_one(&pool)
        .await
        .expect("setup category")
        .get("id");

    let account_id: i32 = sqlx::query("INSERT INTO accounts (name, account_type) VALUES ('Nubank', 'checking') RETURNING id")
        .fetch_one(&pool)
        .await
    ...
    // Action
    let tx = Transaction::insert(&pool, new_tx).await.expect("Failed to insert transaction");

    // Assertion - Verify mapping to database shape
    assert_eq!(tx.category_id, cat_id);
    assert_eq!(
        tx.source,
        TransactionSource::Account { account_id }
    );
    ...
    assert_eq!(tx.date, chrono::NaiveDate::from_ymd_opt(2026, 7, 7).unwrap());

    // Action 2
    let all = Transaction::find_all(&pool, None).await.expect("Failed to find_all");
